...
const EditStudent: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStudent = async () => {
      if (id) {
        const response = await getStudentById(parseInt(id, 10));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (student.studentId) {
      const response = await updateStudent(student.studentId, student);
      if (response.data) {
        navigate('/students');
      }
      if (response.error) setError(response.error);
  ...
  return (
    <div className="max-w-lg mx-auto mt-10 bg-zinc-900 p-8 rounded shadow">
      <h2 className="text-2xl font-bold mb-6 text-white">Edit Student</h2>
